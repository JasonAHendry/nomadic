...
def get_csv_delimiter(csv_path: str, delimiters: List[str] = [",", ";"]):
    ...
    with open(csv_path, "r") as csv:
        ...
        if not len(used) == 1:
            raise MetadataFormatError(f"Found multiple delimiters ({' '.join(used)}) in header: {header.strip()}.")
...
def check_barcode_format(barcode: str, try_to_fix: bool=True) -> str:
    ...
    if not re.match(EXPECTED, barcode):
        ...
        if not try_to_fix:
            raise MetadataFormatError(f"Barcode '{barcode}' has bad format: must conform to '{EXAMPLE}'.")
        ...
        if barcode_int > 96:
            raise MetadataFormatError(f"Barcode '{barcode}' exceeds maximum of {MAX_BARCODE}.")
